...
import minhFoto from './image/Eu.png';
...
const experiences: Experience[] = [
  {
    ...
    role: "QA Tester",
    period: "2025 — Present",
    description: "Leading software quality assurance initiatives for fiscal systems. Executing rigorous manual testing and transitioning towards automated frameworks to ensure stability and compliance.",
  ...
  },
  {
    ...
    period: "2017 — 2023",
    description: "Technical support and infrastructure monitoring. Developed a deep understanding of system architectures and performance bottlenecks, laying the groundwork for a transition to specialized QA.",
...
const skills: Skill[] = [
  {
    ...
    description: "Ensuring new deployments don't disrupt existing functionality through systematic, multi-layered verification processes.",
    icon: "Bug",
    variant: 'surface'
  },
  {
    title: "API Testing",
    description: "Proficient in Postman and Swagger for RESTful service validation.",
    icon: "Code2",
    variant: 'primary'
  },
  {
    title: "SQL / DB",
    description: "Complex queries and data integrity validation within SQL Server environments.",
    icon: "Database",
    variant: 'surface'
  },
  {
    title: "BDD / Gherkin",
    description: "Writing clear, human-readable test scenarios that align business logic with technical execution.",
    icon: "Terminal",
    variant: 'surface'
  },
  {
    title: "Infrastructure Awareness",
    description: "Hands-on experience with AWS cloud monitoring and Windows Server administration to better understand environment-specific issues.",
    icon: "Cloud",
    variant: 'surface'
  },
  {
...
export default function App() {
  ...
  const IconComponent = ({ name, className }: { name: string, className?: string }) => {
    const icons: Record<string, any> = {
      Bug, Code2, Database, Terminal, Cloud, CheckSquare
    };
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb] font-sans text-[#191c1e] selection:bg-emerald-100 selection:text-emerald-900">
      {/* Navigation */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/60 backdrop-blur-md shadow-sm py-3' : 'bg-white/90 backdrop-blur-md shadow-sm py-3'}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-10">
            {['Experience', 'Skills', 'Certifications', 'Contact'].map((item) => (
              <a 
                key={item}
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <p className="text-2xl text-slate-300 font-medium mb-10 flex items-center gap-3">
              Quality Assurance <span className="w-8 h-[1px] bg-emerald-400"></span> Software Quality
            </p>
            <div className="p-8 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 max-w-xl">
              <p className="text-slate-300 leading-relaxed text-lg">
                Profissional dedicado a garantir a entrega de software de alta qualidade por meio de análises criteriosas, casos de teste abrangentes e validações de regras de negócios. Com experiência em ambientes ágeis (Scrum), sempre busco aprimorar processos e colaborar ativamente com a equipe de desenvolvimento para alcançar a excelência em cada projeto.
              </p>
            </div>
          </motion.div>
          ...
          >
            ...
            >
              <div className="inline-flex items-center gap-2 bg-emerald-400 bg-emerald-500/75 px-4 py-1.5 rounded-full mb-8 border border-emerald-400/20">
                <ShieldCheck size={14} />
                <span className="text-[10px] font-bold tracking-[0.2em] uppercase">Precision QA Engineering</span>
              </div>
            </motion.div>
          </motion.div>
        </div>
      ...
      <section id="experience" className="py-32 px-6 md:px-12 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="mb-20">
            <span className="text-emerald-400/20 font-black text-6xl block -mb-8 select-none">01</span>
            <h2 className="text-5xl md:text-6xl font-black text-[#162839] tracking-tighter relative z-10">Jornada Professional</h2>
          </div>
              ...
              >
                <div className="grid md:grid-cols-[1fr_2.5fr] gap-8 md:gap-24">
                  <div className="md:text-right pt-2">
                    <h3 className="text-2xl font-black text-[#162839] mb-1">{exp.company}</h3>
                    <p className="text-blue-600 font-bold text-xs tracking-[0.2em] uppercase">{exp.role}</p>
                    <p className="text-slate-400 text-sm mt-3 font-medium">{exp.period}</p>
                  </div>
                  <div className={`bg-[#f7f9fb] p-10 rounded-2xl border-l-8 ${exp.color} group-hover:shadow-xl group-hover:-translate-y-1 transition-all duration-300`}>
                    <p className="text-slate-600 leading-relaxed text-lg mb-8">
                      {exp.description}
                    </p>
                    <div className="flex flex-wrap gap-3">
                      {exp.tags.map(tag => (
                        <span key={tag} className="px-5 py-1.5 bg-white rounded-full text-[10px] font-bold text-slate-500 uppercase tracking-widest shadow-sm border border-slate-100">
                          {tag}
                        </span>
                      ))}
                    </div>
      ...
      <section id="skills" className="py-32 px-6 md:px-12 bg-[#f2f4f6]">
        <div className="max-w-7xl mx-auto">
          <div className="mb-20 text-center">
            <h2 className="text-5xl font-black text-[#162839] tracking-tighter mb-4">Technical Arsenal</h2>
            <p className="text-slate-500 max-w-2xl mx-auto text-lg">A comprehensive toolkit designed for end-to-end quality assurance across diverse technological environments.</p>
          </div>
            ...
            {skills.map((skill, index) => (
              <motion.div 
                ...
                className={`p-10 rounded-3xl flex flex-col justify-between transition-all duration-300 hover:-translate-y-2 ${
                  skill.variant === 'primary' ? 'bg-[#162839] text-white lg:col-span-1 shadow-2xl' : 
                  skill.variant === 'accent' ? 'bg-emerald-400 text-[#162839] lg:col-span-1' :
              ...
              >
                <div>
                  <div className={`mb-8 w-14 h-14 rounded-2xl flex items-center justify-center ${
                    skill.variant === 'primary' ? 'bg-white/10' : 
                    skill.variant === 'accent' ? 'bg-[#162839]/10' : 
                    'bg-slate-100'
                  }`}>
                    <IconComponent 
                      name={skill.icon} 
                      className={`w-7 h-7 ${
                        skill.variant === 'primary' ? 'text-emerald-400' : 
                        skill.variant === 'accent' ? 'text-[#162839]' : 
                        'text-blue-600'
                      }`} 
                    />
                  </div>
                  ...
                  <p className={`text-sm leading-relaxed ${
                    skill.variant === 'primary' ? 'text-slate-400' : 'text-slate-500'
                  }`}>
                    {skill.description}
                  </p>
                </div>
      ...
      <section id="certifications" className="py-32 px-6 md:px-12 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
            <div>
              <h2 className="text-6xl font-black text-[#162839] tracking-tighter">Certificações</h2>
              <p className="text-slate-500 mt-4 text-lg">Continuous professional development and industry validation.</p>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-10">
            {certifications.map((cert, index) => (
              <motion.div 
                ...
                className="group"
              >
                <div className={`p-6 bg-[#f7f9fb] rounded-[2.5rem] border-[16px] ${cert.borderColor} shadow-2xl transition-transform duration-500 group-hover:scale-[1.02]`}>
                  ...
                  <div className="px-2 pb-2">
                    <h4 className="text-xl font-black text-[#162839] mb-1">{cert.title}</h4>
                    <div className="flex justify-between items-center">
                      <p className="text-sm text-blue-600 font-bold">{cert.issuer}</p>
                      <p className="text-sm text-slate-400 font-bold">{cert.year}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
      ...
      {/* CTA Section */}
      <section id="contact" className="py-32 px-6 md:px-12 bg-[#162839] relative overflow-hidden">
      ...
      <footer className="bg-white border-t border-slate-100 py-8 px-6 md:px-12">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
          <div className="flex flex-col items-center md:items-start">
            <span className="text-2xl font-black tracking-tighter text-[#162839]">JOSIEL SOARES</span>
